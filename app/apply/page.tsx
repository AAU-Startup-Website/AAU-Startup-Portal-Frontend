  ...
  const steps = [
    {
      ...
      validation: (data: any) => {
        ...
        if (!data.targetAudience?.trim()) errors.push("Target audience is required")
    ...
    },
    {
      ...
      validation: (data: any) => {
        const errors: string[] = []
        if (!data.solutionDescription?.trim()) errors.push("Solution description is required")
        if (!data.valueProposition?.trim()) errors.push("Value proposition is required")
        if (!data.productType) errors.push("Product type is required")
        if (!data.developmentStage) errors.push("Development stage is required")
        return errors
      },
    },
    {
      ...
      validation: (data: any) => {
        const errors: string[] = []
        if (!data.marketSize?.trim()) errors.push("Market size estimation is required")
        if (!data.targetMarket?.trim()) errors.push("Target market is required")
        if (!data.competitors?.trim()) errors.push("Competitive analysis is required")
        if (!data.customerAcquisition?.trim()) errors.push("Customer acquisition strategy is required")
      ...
      validation: (data: any) => {
        ...
        if (!data.teamMembers || data.teamMembers.length === 0) {
          ...
          data.teamMembers.forEach((member: any, index: number) => {
            ...
            if (!member.experience?.trim()) errors.push(`Team member ${index + 1}: Experience is required`)
            if (!member.commitment) errors.push(`Team member ${index + 1}: Commitment level is required`)
          })
        }
        return errors
      },
    },
    {
      ...
      validation: (data: any) => {
        ...
        if (!data.sectors || data.sectors.length === 0) errors.push("At least one industry sector is required")
        if (!data.businessStage) errors.push("Business stage is required")
        if (!data.businessModel?.trim()) errors.push("Business model is required")
        return errors
      },
    },
    {
    ...
    },
    {
      ...
      validation: (data: any) => {
        const errors: string[] = []
        if (!data.agreements?.accuracy) errors.push("Please confirm information accuracy")
        if (!data.agreements?.terms) errors.push("Please accept the terms of service")
        if (!data.agreements?.privacy) errors.push("Please accept the privacy policy")
        if (!data.agreements?.communication) errors.push("Please consent to communications")
        return errors
      },
    },
  ]
          ...
          <Card className="text-center">
            <CardHeader>
              ...
              <CardDescription className="text-lg">
                Thank you for applying to the AAU Startups Portal. We've received your application and will review it
                carefully.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">Email notification</p>
                </div>
              </div>
              ...
              <div className="bg-muted/50 p-4 rounded-lg text-left">
                ...
                <ul className="text-sm text-muted-foreground space-y-1">
                  ...
                  <li>• Successful applicants will receive onboarding information</li>
                </ul>
              </div>
    ...
    <div className="min-h-screen bg-background py-16 px-4">
      <div className="container mx-auto">
        ...
        <div className="text-center mb-12">
          <Badge className="bg-aau-gold text-aau-blue mb-4">Applications Open</Badge>
          <h1 className="text-4xl font-bold mb-4">Apply to AAU Startups Portal</h1>
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Join Ethiopia's premier startup incubation program. Get mentorship, funding, and resources to turn your idea
