...
const dbRoleToAppRole = (r?: string | null): AppUser["role"] | undefined => {
  ...
  if (["founder", "mentor", "investor", "admin"].includes(lower)) return lower as AppUser["role"]
  return undefined
}
...
function mapUser(raw: any, row?: any): AppUser | null {
  ...
  const fullName = [first, last].filter(Boolean).join(" ") || md.full_name || md.name || null
  return {
    id: raw.id,
    email: raw.email,
    name: fullName,
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((_event, session) => {
      ...
      if (authUser) {
        fetchUserRow(authUser.id).then((row) => setUser(mapUser(authUser, row)))
      } else {
        setUser(null)
      }
    })
  ...
  const signUp = useCallback(
    async ({ email, password, firstName, lastName, role = "founder", department }: {
      email: string
      password: string
      firstName?: string
      lastName?: string
      ...
      if (authUser) {
        ...
        const { data: upsertData, error: upsertErr } = await supabase
          ...
          .single()
        if (upsertErr) {
          console.warn("[auth] users upsert error", upsertErr.code, upsertErr.message)
        }
  ...
  const setUserRole = useCallback(async (role: AppUser["role"]) => {
    if (!user) return
    const dbRole = appRoleToDbRole(role)
    const { error: updateErr } = await supabase.from("users").update({ role: dbRole }).eq("id", user.id)
    if (updateErr) {
  ...
  return (
    <AuthContext.Provider
      value={{ user, isAuthenticated, loading, login, signUp, logout, setUserRole, refreshProfile }}
    >
      {children}
    </AuthContext.Provider>
  )
